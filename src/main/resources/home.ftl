        ...
        <div class="container">
            ...
            <#list items as item>
                ...
                <div class="card">
                    ...
                    <a href="item/id/${item.id}" class="badge badge-warning">${item.name} $${item.price}</a>
                    <#if item.description??><p class="card-text">${item.description}</p></#if>
                    <#if user?? ><a href="/gift_card_service/u/buy/id/${item.id}" class="mt-3 btn btn-primary btn-md">Buy</a></#if>
                </div>
            </#list>
            </div>
